...
logging.basicConfig(
    level=logging.INFO,  # Nível de log (INFO, WARNING, ERROR)
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(),  # Log no console
        logging.FileHandler("app.log", encoding="utf-8")  # Log em arquivo
    ]
)
...
# Gerando os números da grid triângulo
def get_valid_pairs(grid):
    logging.info("Gerando pares válidos para a grid fornecida.")
    valid_pairs = set()
    rows = [
        grid[0:5],    
        grid[5:9],    
    ...
    # Processamento dos pares válidos (horizontal, vertical, diagonal, etc.)
    # (Mantendo o código original aqui)
...
def generate():
    try:
        logging.info("Recebendo requisição para geração de combinações.")
        data = request.json
        if not data:
        ...
        if not numbers or not num_dezenas:
            logging.warning("Parâmetros incompletos na requisição.")
            return jsonify({'error': 'Parâmetros incompletos'}), 400
        ...
        valid_pairs = get_valid_pairs(numbers)
        logging.info(f"Pares válidos gerados: {valid_pairs}")
        
        if not (6 <= num_dezenas <= 20):
            logging.warning("Número de dezenas fora do intervalo permitido (6-20).")
            return jsonify({'error': 'Número de dezenas deve estar entre 6 e 20'}), 400
        ...
        if total_possible > MAX_COMBINATIONS:
            import random
            valid_pairs = random.sample(valid_pairs, min(len(valid_pairs), 20))
            logging.info("Número de combinações muito grande. Limitando pares válidos.")
        ...
        logging.info(f"Total de jogos válidos gerados: {len(jogos_validos)}")
        return jsonify({
            'combinations': jogos_validos,
            'total': len(jogos_validos),
        ...
        logging.error(f"Erro ao gerar combinações: {str(e)}")
        return jsonify({'error': str(e)}), 400

@app.route('/download/<format>', methods=['POST'])
def download(format):
    try:
        logging.info(f"Recebendo requisição para download no formato {format}.")
        data = request.json
        combinations = data.get('combinations', [])
        num_dezenas = data.get('num_dezenas', 6)
        
        # Implementação original do método
        # (Mantendo o código original aqui para tratar TXT, XLSX, HTML)
        
    except Exception as e:
        logging.error(f"Erro ao processar download: {str(e)}")
        return jsonify({'error': str(e)}), 400

@app.route('/check-results', methods=['POST'])
def check_results():
    try:
        logging.info("Recebendo requisição para verificar resultados.")
        data = request.json
        generated_games = data['games']
        ...
        try:
            ...
            logging.error(f"Erro ao acessar API da loteria: {str(e)}")
            return jsonify({'error': f'Erro ao acessar API da loteria: {str(e)}'}), 500
        ...
        logging.info(f"Total de jogos com resultados: {len(results)}")
        return jsonify({
            'results': sorted(results, key=lambda x: max(r['acertos'] for r in x['resultados']), reverse=True)
        })
